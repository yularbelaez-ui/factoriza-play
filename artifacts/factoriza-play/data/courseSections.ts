export type SectionStatus = "diagnostico" | "disponible" | "activo";

export interface SectionTopic {
  label: string;
  topicId: string | null;
}

export interface CourseSection {
  id: string;
  number: string;
  title: string;
  subtitle: string;
  icon: string;
  color: string;
  lightColor: string;
  borderColor: string;
  topics: SectionTopic[];
  status: SectionStatus;
}

export const COURSE_SECTIONS: CourseSection[] = [
  {
    id: "saberes",
    number: "01",
    title: "Zona de Repaso",
    subtitle: "Saberes previos",
    icon: "🧮",
    color: "#7c3aed",
    lightColor: "#f5f3ff",
    borderColor: "#ddd6fe",
    topics: [
      { label: "Números naturales y operaciones",   topicId: "s1-naturales"   },
      { label: "Números decimales y operaciones",   topicId: "s1-decimales"   },
      { label: "Números enteros y negativos",       topicId: "s1-enteros"     },
      { label: "Fracciones: equivalencias y operaciones", topicId: "s1-racionales"  },
      { label: "Números irracionales",              topicId: "s1-irracionales"},
      { label: "Números reales",                    topicId: "s1-reales"      },
      { label: "Potencias y propiedades",           topicId: "s1-potencias"   },
      { label: "Factores primos",                   topicId: "s1-factores"    },
    ],
    status: "diagnostico",
  },
  {
    id: "algebra",
    number: "02",
    title: "Introducción al Álgebra",
    subtitle: "Conceptos fundamentales",
    icon: "✏️",
    color: "#2563eb",
    lightColor: "#eff6ff",
    borderColor: "#bfdbfe",
    topics: [
      { label: "Notación algebraica y grado de un término", topicId: "s2-notacion" },
      { label: "Expresión y término algebraico",       topicId: "s2-expresion"    },
      { label: "Grado de un término",                  topicId: "s2-grado"        },
      { label: "Clasificación de expresiones algebraicas", topicId: "s2-clasificacion" },
      { label: "Términos semejantes y valor numérico", topicId: "s2-semejantes"   },
    ],
    status: "disponible",
  },
  {
    id: "operaciones",
    number: "03",
    title: "Operaciones Algebraicas",
    subtitle: "Suma, resta, multiplicación y división",
    icon: "⚙️",
    color: "#059669",
    lightColor: "#f0fdf4",
    borderColor: "#bbf7d0",
    topics: [
      { label: "Suma y resta algebraica",              topicId: "s3-suma-resta"       },
      { label: "Multiplicación algebraica",            topicId: "s3-multiplicacion"   },
      { label: "División algebraica",                  topicId: "s3-division"         },
      { label: "Productos notables",                   topicId: "s3-productos"        },
    ],
    status: "disponible",
  },
  {
    id: "factorizacion",
    number: "04",
    title: "Factorización",
    subtitle: "Una secuencia progresiva de patrones y casos",
    icon: "🔍",
    color: "#d97706",
    lightColor: "#fffbeb",
    borderColor: "#fde68a",
    topics: [
      { label: "Módulo 1: Reconocimiento de patrones",               topicId: null },
      { label: "Módulo 2: Factor común (numérico, literal y mixto)", topicId: null },
      { label: "Módulo 3: Factor común por agrupación",              topicId: null },
      { label: "Módulo 4: Trinomio cuadrado perfecto",               topicId: null },
      { label: "Módulo 5: Diferencia de cuadrados perfectos",        topicId: null },
      { label: "Módulo 6: Trinomio de la forma x²+bx+c",             topicId: null },
      { label: "Módulo 7: Cubo perfecto de binomios",                topicId: null },
      { label: "Módulo 8: Suma o diferencia de cubos perfectos",     topicId: null },
    ],
    status: "activo",
  },
];
